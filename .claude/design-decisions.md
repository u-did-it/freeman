...
## DD-017 — Environment/collection variable values: unbounded validation + TEXT column
**Decision:** `variables.*.value` in `StoreEnvironmentRequest`/`UpdateEnvironmentRequest` had a `max:2048` cap — removed (now just `nullable, string`), matching `CollectionController::updateVariables()`'s inline validation which was already unbounded. Also widened `environment_variables.value` and `collection_variables.value` from `string` (VARCHAR) to `text` via a new migration (`2026_08_30_000001_widen_variable_value_columns_to_text.php`), matching `requests.body`.
